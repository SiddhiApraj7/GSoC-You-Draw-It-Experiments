  ...
  const on_end = function(){
    ...
    if(line_status === 'done'){
      ...
      if(state.show_finished){
        ...
        removeHint(state);
        //siddhi
        showHint(state, "Well done! Compare your estimated line and regression line", 'blue');
      ...
      //siddhi
      showHint(state, "Move slow to pick all points .. Orange region shows region for selection", 'red');
    }
  };
...
function showHint(state, tipText, mcolor) {
...
function get_user_line_status({drawable_points, free_draw}){
 ...
 // console.log("num_points:"+num_points+"num_filled:"+num_filled+"num_starting_filled:"+num_starting_filled);
  /*console.log(num_points);
  console.log("num_filled:");
  console.log(num_filled);
  console.log("num_starting_filled:");
  console.log(num_starting_filled);*/
...
/*var points = [
        ...
        var Gen = d3.line()
            .x((p) =>  scales.x(p.xpoint))
            .y((p) =>  scales.y(p.ypoint))
            .curve(d3.curveNatural);
...
/*
  ...
  const cdots = state.svg.selectAll("path").data(df)
...
function draw_rectangle({svg, drawable_points, line_data, draw_start, width, height, free_draw, x_by}, scales){
    ...
    //console.log("drawing :"+drawSpace_start+"end:"+drawSpace_end+"height:"+state.h)
    draw_region
...
/*
  if(drawable_points == "partial"){
      var df = drawable_points.filter(function(d){return (d.x<=points_end)});
    } else {
      var df = drawable_points;
    }
  const cdots = state.svg.selectAll("path").data(df)
